...
export function SalesReportView({ initialData, orgId }: SalesReportViewProps) {
  ...
  const invoiceCount = data.summary.reduce(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Sales totals and customer breakdown
            </p>
          </div>
        </div>
              ...
              <TableBody>
                {data.summary.map((row) => (
                  <TableRow key={row.period}>
                    ...
                    <TableCell className="nums text-right text-slate-700 dark:text-slate-300">
                      {formatCurrency(row.subtotal, true)}
                    </TableCell>
                    <TableCell className="nums text-right text-slate-700 dark:text-slate-300">
                      {formatCurrency(row.taxAmount, true)}
                    </TableCell>
                ...
                <TableRow className="border-t-2 border-slate-200 font-semibold hover:bg-transparent dark:border-slate-700 dark:hover:bg-transparent">
                  ...
                  <TableCell className="nums text-right text-slate-900 dark:text-slate-100">
                    {formatCurrency(data.totals.subtotal, true)}
                  </TableCell>
                  <TableCell className="nums text-right text-slate-900 dark:text-slate-100">
                    {formatCurrency(data.totals.taxAmount, true)}
                  </TableCell>
                  <TableCell className="nums text-right text-slate-900 dark:text-slate-100">
