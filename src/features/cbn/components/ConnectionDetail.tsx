  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
        <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-teal-100 text-base font-bold text-teal-700">
          {otherOrg.name.slice(0, 2).toUpperCase()}
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-slate-900">{otherOrg.name}</h3>
          {otherOrg.businessId && (
            <p className="font-mono text-xs text-slate-400">{otherOrg.businessId}</p>
          )}
      ...
      {connection.status === "accepted" && (
        <div>
          ...
          {saved && (
            <p className="mt-3 text-sm text-green-600">Permissions saved.</p>
          )}
...
function EventsTab({
  ...
  return (
    <ol className="relative space-y-4 border-l border-slate-200 pl-4" aria-label="Connection events">
      {events.map((event) => {
        ...
        return (
          <li key={event.id} className="relative">
            <div
              className={`absolute -left-[21px] flex h-5 w-5 items-center justify-center rounded-full border-2 border-white ${
                isSuccess
                  ? "bg-green-100"
                  : isFailed
                    ? "bg-red-100"
