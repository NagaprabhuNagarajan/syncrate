...
type DbUnitInsert = Database["public"]["Tables"]["units"]["Insert"];
...
function mapUnit(row: DbUnit): Unit {
  return {
    id: row.id,
...
export class UnitRepository {
  ...
  async create(input: DbUnitInsert): Promise<Unit | null> {
    const { data, error } = await this.supabase
      .from("units")
      .insert(input)
      .select("*")
