...
export function ProposedActionCard({
  ...
  if (!parsed.success) {
    return (
      <div className="border-warning/40 bg-warning/5 mt-3 rounded-xl border p-4 text-sm text-slate-700">
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 py-3">
        ...
        {doc.customerName && (
          <span className="text-xs text-slate-500">{doc.customerName}</span>
        )}
      </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tfoot>
            <tr className="border-t border-slate-100">
              <td
                className="px-4 py-2 text-right text-xs font-medium uppercase text-slate-500"
                colSpan={4}
              >
                Subtotal (excl. tax)
              </td>
