...
interface RecordSupplierPaymentDialogProps {
  ...
  readonly outstandingInvoices?: readonly OutstandingPurchaseInvoice[];
...
export function RecordSupplierPaymentDialog({
  ...
  function addAllocation(): void {
    setAllocations((prev) => [
      ...prev,
      { key: `alloc-${Date.now()}`, purchaseInvoiceId: "", amount: "" },
    ]);
  }
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>): void {
    ...
    const validAllocations = allocations
      .filter((a) => a.purchaseInvoiceId && Number(a.amount) > 0)
      .map((a) => ({
                      ...
                      <div className="flex-1">
                        {outstandingInvoices.length > 0 ? (
                          <select
                            ...
                            className={cn(inputClass, "appearance-none")}
                            aria-label={`Purchase invoice for allocation ${index + 1}`}
                          >
